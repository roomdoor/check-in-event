...
| 방식 | 지연이 유지되는 한계 | 초당 저장 한계 |
| --- | --- | --- |
| **DB 락** | 100 RPS (초당 90건) | **205/s** |
| **Redis** | 1,600 RPS (초당 1,445건) | **2,858/s 이상** |

**쓸 수 있는 구간이 16배 차이난다.** 천장끼리는 14배지만 그쪽은 둘 다 "언젠가 흡수는 된다"는 값이라 덜 중요하다.

**두 방식 모두 천장보다 지연이 먼저 무너진다.** DB 락은 초당 200건에서 이미 p95 1.25초, Redis 는 2,000 RPS 에서 174ms 다. 그 앞까지는 각각 11ms, 5ms 로 평평하다.
...
**병목은 양쪽 다 예상 밖이었다.** DB 락은 스레드가 아니라 행 락이고, Redis 는 Redis 가 아니라 **앱**이다 — 같은 Lua 스크립트를 Redis 에 직접 쏘면 초당 55,000건을 하는데 앱을 거치면 3,000~4,700건이다. 그래서 Redis 쪽은 앱을 늘리면 더 간다.
...
800 에서도 안 꺾였으므로 부하를 두 배씩 올려 다시 쟀다.

| RPS | 저장 | 초당 | p95 | 드레인 |
| --- | --- | --- | --- | --- |
| 800 | 43,372 | 722 | 5.35ms | 0초 |
| 1600 | 86,711 | 1,445 | **4.78ms** | 0초 |
| 2000 | 108,335 | 1,806 | **174ms** | 0초 |
| 2400 | 129,776 | 2,163 | 485ms | 0초 |
| 2800 | 150,886 | 2,515 | 749ms | 0초 |
| 3200 | 171,499 | 2,858 | 990ms | 0초 |
| 6400 | 180,998 | 3,016 | 2,463ms | 0초 |

**무릎이 1600 과 2000 사이에 있다.** 5ms 에서 174ms 로 36배 뛴다. 그 뒤로는 거의 선형으로 자란다 — 대기열이 자라는 모양이다.

**처리량은 3200 까지 도착률을 따라간다**(2,858 대 예상 2,880, 99.2%). 6400 회차의 3,016/s 는 천장으로 읽으면 안 된다. 같은 설정을 다시 돌렸을 때 그 회차만 280,329건(초당 4,672)이 나왔다 — 3200 이하는 0.5% 안에서 재현되는데 **포화 지점만 55% 벌어진다.**

**드레이너는 범인이 아니다.** 초당 3,016건을 저장하는 회차까지 드레인이 0초다. batch 1600 / delay 100ms 조합은 승인 경로보다 계속 앞서 있었고, 그 자체의 천장은 아직 안 잡혔다.
...
## 4. Redis 쪽 병목은 Redis 가 아니라 앱이다

2 장이 무릎을 찾았지만 **어디가 막혔는지**는 말하지 않았다. 후보가 앱과 Redis 둘인데, 답에 따라 대응이 정반대다 — 앱이면 옆으로 늘리면 되고, Redis 면 Lua 가 단일 스레드라 늘려도 소용없다.

앱을 두 대로 늘려 재려면 인프라를 바꿔야 해서, 더 싼 실험 둘로 대신했다.

**Redis 에 같은 스크립트를 직접 쏘았다.** 앱을 안 거치고 `redis6-benchmark` 로, 커넥션 50개, `__rand_int__` 로 userId 를 매번 다르게 줘서 승인 전체 경로를 타게 했다.
...
**12~18배 차이다.** Redis 는 실제 부하에서 자기 용량의 10%도 안 쓰고 있었다. 지연도 같은 곳을 가리킨다 — Redis 는 p95 1.13ms 에 답하는데 앱은 낮은 부하에서도 4.3~5ms 다. **3~4ms 가 앱에서 쓰인다.**

**CPU 는 양쪽 다 남았다.** 부하 중 1초 간격으로 찍은 값이다.

| 회차 | SUT (앱, 8 vCPU) | support (MySQL+Redis, 4 vCPU) |
| --- | --- | --- |
| 800 | 11% | 9% |
| 1600 | 21% | 13% |
| 3200 | 27% | 21% |
...
이 표만으로는 결론이 안 난다. **호스트 전체 평균이라 단일 스레드 병목이 안 보인다** — Redis 는 싱글 스레드라 4 vCPU 호스트에서 코어 하나가 꽉 차도 25% 로 찍힌다. 위의 벤치가 그 가능성을 지웠다.

**그래서 앱을 늘리면 통한다.** 승인 판정이 Redis 에 있어 앱은 상태가 없고, Redis 에 12~18배 여유가 있으니 한동안 그쪽이 벽이 되지 않는다. 다만 **앱의 어느 부분인지는 아직 모른다** — CPU 가 31% 인데 못 받는다는 건 스레드나 대기가 막고 있다는 뜻이다.

원본: [`loadtest/results/ec2/bottleneck`](loadtest/results/ec2/bottleneck)

---

## 미해결

**Redis 승인 경로의 천장은 아직 숫자로 못 박는다.** 3200 RPS(초당 2,858건)까지 도착률을 따라가는 것은 확실하다. 그 위는 같은 설정을 두 번 돌렸을 때 초당 3,016건과 4,672건이 나왔다 — **포화 지점만 55% 벌어진다.** 포화 아래는 0.5% 안에서 재현된다.

**앱의 어느 부분이 막는지 모른다.** CPU 31% 에서 못 받으니 스레드나 대기다 — Tomcat 워커, 커넥션 풀, JVM 중 하나. 다음에 잴 것.

**앱 여러 대는 안 재봤다.** "앱이 병목이므로 늘리면 된다"는 추론이고, 실제로 선형으로 느는지는 확인 안 했다. 인프라가 SUT 를 한 대만 만든다.

**드레이너의 천장도 여전히 모른다.** batch 1600 / delay 100ms 는 초당 3,016건까지 드레인 0초로 따라왔다. 승인 경로가 먼저 막혀서 드레이너를 끝까지 밀어보지 못했다.
...
**거의 다 n=1 이다.** 커밋된 32회차 중 반복이 있는 건 `ceiling.env` 의 네 rate 뿐이고(두 번씩), 그 반복이 포화 지점의 재현 불가를 드러냈다. 나머지는 한 번씩이다.
...
`loadtest/results/ec2/ceiling` · `ceiling-narrow` — 2 장의 두 번째 표. 두 배씩 올려 구간을 찾고(4회차), 1600~3200 사이를 400 간격으로 좁혔다(3회차).

`loadtest/results/ec2/bottleneck` — 4 장. `ceiling.env` 를 CPU 계측과 함께 다시 돌린 4회차와, Redis 단독 벤치·CPU 표를 정리한 [`FINDINGS.md`](loadtest/results/ec2/bottleneck/FINDINGS.md). 벤치 수치는 `result.json` 형식에 안 들어가서 따로 적었다.
