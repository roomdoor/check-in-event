...
**앱이 놀고 있었다.** 그런데 그때 측정값은 p95 990ms 였다. 앱이 한가한데 응답이 1초 걸린다는 건 앱이 막는 게 아니라는 뜻이고, 그 모순이 5 장으로 이어졌다.
...
>
> 이후 회차부터는 `result.json` 의 `load.warmed_by` 로 구분된다. `"none"` 은 스윕이 워밍업을 돌렸는데 부하가 안 나간 것, `"unknown"` 은 손으로 `run.sh` 를 돌려 알 수 없는 것, 나머지는 그 길이만큼 데운 것이다. 여기 있는 35회차는 그 필드가 생기기 전이라 전부 없다.
