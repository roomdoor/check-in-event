...
| 방식 | 초당 승인 | 초당 저장 | p95 |
| --- | --- | --- | --- |
| **DB 락** | 205 | **205** (승인과 같다) | 200 RPS 에서 1.25초 |
| **Redis** | **5,761 이상** | **약 4,165** | **2.15ms** |

**두 숫자를 구분해야 한다.** DB 락은 요청 안에서 저장까지 끝내므로 승인과 저장이 같은 값이다. Redis 는 승인만 하고 답하므로 두 값이 갈린다 — 엔드포인트가 받아내는 양(5,761)과 원장에 실제로 앉는 양(4,165)이 다르다.

**같은 기준으로 비교하면 저장이 20배, 승인이 28배다.** 다만 두 행의 조건이 같지 않다 — **DB 락 수치는 콜드 JVM 회차이고 Redis 는 데워진 회차다.** db 를 따뜻한 상태로 다시 재지 않았다(미해결 참고).

**Redis 의 두 숫자는 성격이 다르다.** 승인 5,761 은 **천장이 아니다** — 6,400 RPS 에서 요청을 하나도 안 흘리고 2밀리초에 답했으니 한계는 그 위 어딘가다. 반면 저장 4,165 는 **밀린 회차에서 나온 실제 천장**이다(3 장).

**막는 자리가 예상과 달랐다.** DB 락은 스레드가 아니라 **행 하나**다 — 모든 요청이 같은 `events` 행을 트랜잭션 내내 잠그니 이미 직렬이고, 스레드를 늘려도 그 구간은 안 넓어진다. Redis 쪽은 Redis 도 앱도 아니었다. 같은 Lua 를 Redis 에 직접 쏘면 초당 55,000건이고, 앱은 6,400 RPS 를 2ms 에 받는다. **처음 밀린 건 `CheckInStreamWriter`(Redis→MySQL 스케줄러)였고 초당 4,165건이다.**
...
**이 수치들은 데워진 앱에서 잰 것이다.** 처음에는 아니었다 — 스윕이 회차마다 앱을 재기동해서 매 회차가 갓 뜬 JVM 을 재고 있었고, 그 상태의 숫자를 "앱의 한계"로 읽었다. 같은 6,400 RPS 가 따뜻하면 p95 2.15ms, 재기동 직후면 1,474ms 다. 5 장에 따로 적었다.

> 정원을 부하보다 크게 둔 조건이다 — 회차마다 다르고, 위 Redis 수치는 정원 2,000,000 회차다. 거절은 저장하지 않으므로, 정원이 작으면 대부분이 거절이 되어 정작 재려던 저장 부하가 안 걸린다.
...
800 에서도 안 꺾여서 부하를 올렸다. **데워진 앱에 2분씩** 준 결과다.

| RPS | 요청 | 승인 | 초당 승인 | p95 | 드레인 | 버려짐 |
| --- | --- | --- | --- | --- | --- | --- |
| 3200 | 383,031 | 345,108 | 2,876 | **2.20ms** | 2초 | 971 |
| 6400 | 768,003 | 691,367 | **5,761** | **2.15ms** | **46초** | **0** |

> `초당 승인` 은 부하 시간으로만 나눈 값이다. 저장은 드레인까지 걸리므로 그보다 낮다 — 6400 회차는 120초 + 46초에 691,367건이라 **초당 4,165건**이고, 아래에서 그 계산을 한다. 1 장의 표는 두 값을 나눠 적었다.

**6,400 RPS 를 요청 하나도 안 흘리고 2밀리초에 답한다.** 요청 수가 `6400 × 120초 = 768,000` 과 정확히 맞는다 — 부하가 전부 갔다는 뜻이다. 승인 경로의 천장은 **여기서도 안 나왔다.**

> 두 회차의 VU 설정이 다르다 — 3200 은 사전 2,000/최대 8,000, 6400 은 4,000/16,000 이다. 그래서 **버려짐 열은 앱의 성질이 아니라 부하 생성기 쪽 값**이다. 낮은 rate 쪽이 971건을 흘린 게 그 때문이다. 6400 회차의 0 은 넉넉히 준 덕이고, 그 회차에서 읽을 건 `768,003 = 6400 × 120` 이라는 사실이다.

**대신 드레이너가 처음 밀렸다.** 6400 회차에서 부하가 끝난 뒤 남은 걸 빼는 데 46초가 걸렸다. 거기서 처리량이 나온다 — `46D = 691,367 − 120D` 이므로 **초당 약 4,165건**이다.

**이 4,165 가 유일한 포화 측정이다.** 1분짜리 6400 회차는 드레인이 2초로 끝났다 — 밀리지 않았다는 뜻이고, 그 회차가 말해주는 건 드레이너가 **초당 5,425건 이상을 따라잡았다**는 하한뿐이다. 천장이 아니다.

둘을 나란히 놓으면 어긋난다. 작은 회차에서 5,425 이상을 하던 드레이너가 큰 회차에서 4,165 로 잰다. **테이블이 커지면서 인덱스 유지 비용이 올라간 것**으로 보이는데, 각각 n=1 이라 확인은 못 했다. 확실한 건 **부하가 길어질수록 느려진다**는 방향이다.
...
승인 경로가 어디서 막히는지 찾다가 **막히지 않는다**는 걸 알게 된 과정이다. 후보가 Redis 와 앱 둘이었고, 답에 따라 대응이 정반대다 — 앱이면 옆으로 늘리면 되고, Redis 면 Lua 가 단일 스레드라 늘려도 소용없다.
...
| 실제 부하에서 통과한 양 | 5,761 | 2.15ms |

**Redis 는 자기 용량의 10%만 쓰고 있었다.** 명령 6개짜리 스크립트가 PING 의 1/3 인 건 자연스럽고, 병목의 징후가 아니다.

**앱도 아니었다.** 부하 중에 스레드 덤프를 떴다.
...
호스트 CPU 표만으로는 결론을 못 낸다는 점도 적어둔다. **전체 평균이라 단일 스레드 병목이 안 보인다** — Redis 는 싱글 스레드라 4 vCPU 호스트에서 코어 하나가 꽉 차도 25%로 찍힌다. 위의 벤치와 스레드 덤프가 그 가능성을 지웠다.
...
앱이 유휴인데 p95 가 1초였던 이유다. **같은 앱, 같은 6,400 RPS. JVM 상태가 다르다.**

| 앱 상태 | 부하 | p95 | 버려진 요청 |
| --- | --- | --- | --- |
| **따뜻함** (38만 건 처리 후) | 2분 | **2.15ms** | **0** |
| **재기동 직후** | 1분 | **1,474ms** | **10,562** |

**686배다.** 무너지는 건 지연과 배달률이고, 그게 이 표가 말하는 전부다.

**처리량은 이 표로 비교하지 않는다.** 두 회차의 부하 길이가 다르고(2분 대 1분), 드레인까지 보정하면 따뜻한 쪽이 오히려 낮다 — 초당 4,165 대 5,425 이상. 뒤엣것은 그 회차가 안 밀려서 나온 하한이라 애초에 같은 종류의 값도 아니다. **워밍업이 바꾸는 건 지연과 배달률이지 처리량이 아니다**가 이 표에서 읽을 내용이다.

JVM 은 처음에 코드를 한 줄씩 해석하다가, 자주 도는 부분을 기계어로 컴파일한다. 그 전환에 수천~수만 번의 실행이 필요하다. 클래스 로딩도 처음 쓸 때 일어난다. **응답이 2밀리초짜리인 서비스에서는 이 비용이 측정값 자체를 삼킨다.**

**선착순은 이 함정에 정확히 걸린다.** 정각 피크를 대비해 미리 스케일아웃 하면, 그 인스턴스들이 **차가운 채로 피크를 맞는다.** 평소 트래픽이 낮아 JIT 이 덜 올라가 있으면 더 그렇다. 위 표의 콜드 수치는 버릴 값이 아니라 **배포·스케일아웃 중에 피크가 오면 이렇게 된다**는 값이다.

**이 저장소는 한동안 콜드만 재놓고 정상 상태라고 불렀다.** 스윕이 회차마다 앱을 재기동하기 때문이다(드레이너 설정을 바꾸려면 필요하다). 그래서 "1,600~2,000 RPS 에서 무릎", "승인 경로 천장 초당 3,000건" 같은 결론이 나왔는데, 전부 워밍업을 잰 것이었다. 2 장의 표가 그걸 고친 값이다.

원본: [`loadtest/results/ec2/warmup`](loadtest/results/ec2/warmup)

---

## 미해결

**승인 경로의 천장을 아직 못 찾았다.** 6,400 RPS 에서도 p95 2.15ms 에 버린 요청이 0이다. 더 올리려면 부하 호스트를 늘려야 한다.

**`warmup` 을 뺀 커밋 결과는 전부 콜드 JVM 이다.** 스윕 다섯 디렉터리(`compare`·`drainer`·`ceiling`·`ceiling-narrow`·`bottleneck`)와 스윕 이전 단발 회차 하나(`redis-rate400-dup0.2-…`)가 그렇다. 회차마다 앱을 재기동하기 때문이다. 5 장을 알고 나서 다시 읽어야 한다.

- **처리량 수치는 비교적 견딘다.** 같은 값에 눕는 평탄부(db 의 205/s)는 워밍업으로 안 생긴다
- **지연 수치는 못 믿는다.** 특히 높은 rate 의 p95 는 워밍업이 섞여 있다

**db 쪽은 따뜻한 상태로 다시 안 쟀다.** 1 장의 205/s 평탄부는 행 락 직렬화라는 구조적 설명이 있어 그대로 두지만, **"쓸 수 있는 건 100 RPS 언저리"는 콜드 회차에서 나온 지연 판단**이라 다시 재야 한다.

**드레이너 천장이 한 번만 잡혔다.** 2분 회차가 밀리면서 초당 4,165건이 나왔다. 1분 회차는 안 밀려서 **5,425건 이상**이라는 하한만 준다 — 두 값의 종류가 다르고, 어긋나는 방향이 테이블 크기를 가리키지만 확인 안 했다.

**앱 여러 대는 안 재봤다.** 승인 경로에 여유가 있으니 지금은 급하지 않지만, 늘렸을 때 선형인지는 모른다. 인프라가 SUT 를 한 대만 만든다.
...
**같은 조건의 반복이 하나도 없다.** 같은 rate 를 여러 번 돈 회차는 많다 — 35회차 중 `redis 800` 만 9회, `redis 200` 이 7회다. 하지만 드레이너 설정·부하 길이·VU 수·JVM 상태가 회차마다 달라서 **반복 측정이라고 부를 수 있는 쌍이 없다.** `ceiling.env` 를 두 번 돌린 것이 가장 가까웠고, 그마저 포화 지점에서 55% 벌어졌다.
...
JVM 은 처음에 느리다 — 코드를 해석하다가 자주 도는 부분만 기계어로 컴파일하고, 클래스도 처음 쓸 때 로딩한다. **응답이 밀리초 단위인 서비스에서는 이 비용이 측정값보다 크다.**

스윕은 회차마다 앱을 재기동한다. 드레이너 설정을 바꾸려면 필요하고, 지난 회차 상태가 남지 않게 하려면도 필요하다. 그래서 **재기동한 회차는 본 측정 전에 부하를 한 번 흘려보낸다.**

그 값이 얼마나 큰지는 5 장에 있다 — 같은 부하에서 p95 가 686배 차이났다. 이 저장소가 한동안 콜드를 정상 상태로 착각하고 발표한 값들이 있고, 그것도 거기 적어뒀다.

**콜드 수치가 필요 없다는 뜻은 아니다.** 배포나 스케일아웃 중에 피크가 오면 실제로 그 상태다. 다만 **어느 쪽을 쟀는지 이름을 붙여야** 한다.
...
`loadtest/results/ec2/ceiling` · `ceiling-narrow` — 콜드 상태로 두 배씩 올려 구간을 찾고(4회차), 1600~3200 사이를 400 간격으로 좁힌(3회차) 회차들. **이 수치는 본문 어느 표에도 안 쓰였다** — 여기서 나온 "무릎"이 워밍업 아티팩트였기 때문이다. 5 장의 근거로 남긴다.
...
`loadtest/results/ec2/warmup` — 2 장과 5 장. 스윕을 거치지 않고 손으로 돌린 3회차다. 따뜻한 앱에 3200·6400 RPS 를 2분씩, 그리고 같은 6400 을 재기동 직후 1분. **이 저장소에서 워밍업된 상태로 잰 유일한 수치들이다.**

> **35회차 중 데워진 건 2회차뿐이다.** `warmup/` 의 2분짜리 3200·6400 회차가 그것이고, 같은 디렉터리의 나머지 하나(`…-081740`)는 **일부러 재기동 직후에 잰 콜드 회차**다 — 5 장의 1,474ms 가 거기서 나온다. 나머지 32회차도 전부 콜드다.
>
> 콜드 회차를 남겨두는 건 5 장의 근거이자, 배포·스케일아웃 중 피크가 어떤 모양인지 보여주기 때문이다. **정상 상태 수치로 인용하면 안 된다.**
>
> 이후 회차부터는 `result.json` 의 `load.warmed_by` 로 구분된다. `"off"` 는 워밍업을 끄고 일부러 콜드를 잰 것, `"none"` 은 돌렸는데 부하가 안 나간 것, `"unknown"` 은 손으로 `run.sh` 를 돌려 알 수 없는 것, `"30s"` 는 그 길이만큼 한 번, `"30sx3"` 은 요청 수가 모자라 3회 반복한 것이다. 낮은 rate 와 db 모드는 반복형이 흔하다.
>
> 길이만으로는 충분한지 알 수 없어 `load.warmup_requests` 도 같이 남는다. `load.warmup_above_threshold` 는 그게 설정한 기준(기본 2만)을 넘었는지인데, **넘었다고 충분하다는 뜻은 아니다** — 그 기준 자체가 검증된 값이 아니다. 확인된 웜 상태는 38만 요청 뒤였고, 37만을 콜드로 받은 회차는 여전히 p95 1,474ms 였다.
>
> `load.warmup_drain_capped` 가 `true` 면 워밍업이 남긴 걸 다 못 빼고 본 회차가 시작됐다는 뜻이다. 밀린 항목 자체는 상태 초기화가 지우므로 그 회차 `drain.seconds` 에 안 들어온다 — 남는 건 드레이너 경합과 커진 테이블이다.
>
> 여기 있는 35회차는 이 필드들이 생기기 전이라 전부 없다.
