...
>
> 길이만으로는 충분한지 알 수 없어 `load.warmup_requests` 도 같이 남는다 — 낮은 rate 에서 30초면 몇 천 건뿐이라 JIT 이 덜 올라간다. 그 숫자가 작으면 웜이라고 적혀 있어도 지연을 믿으면 안 된다. `load.warmup_drain_capped` 가 `true` 면 워밍업 잔량이 그 회차 드레인에 섞였다는 뜻이다.
>
