...
**막는 자리가 예상과 달랐다.** DB 락은 스레드가 아니라 **행 하나**다 — 모든 요청이 같은 `events` 행을 트랜잭션 내내 잠그니 이미 직렬이고, 스레드를 늘려도 그 구간은 안 넓어진다. Redis 쪽은 Redis 도 앱도 아니었다. 같은 Lua 를 Redis 에 직접 쏘면 초당 55,000건이고, 앱은 6,400 RPS 를 2ms 에 받는다. **처음 밀린 건 `CheckInStreamWriter`(Redis→MySQL 스케줄러)였고 초당 4,000~5,400건이다.**
...
1분짜리 회차에서는 같은 계산이 초당 5,425건을 준다. 2분 회차가 더 느린 건 **테이블이 커지면서 인덱스 유지 비용이 올라간 것**으로 보이는데, 각각 n=1 이라 단정하지 않는다. 초당 4,000~5,400 구간으로 읽는 게 맞다.
...
**처리량은 이 표로 비교하지 않는다.** 두 회차의 부하 길이가 다르고(2분 대 1분), 드레인까지 넣어 보정하면 따뜻한 쪽이 오히려 낮다 — 초당 4,165 대 5,425. 테이블이 커지며 저장이 느려진 것으로 보이는데 각각 n=1 이라 단정하지 않는다. **워밍업이 바꾸는 건 지연이지 처리량이 아니다**가 이 표에서 읽을 내용이다.
...
**드레이너 천장이 n=1 씩 두 개다.** 2분 회차는 초당 4,165건, 1분 회차는 5,425건을 준다. 테이블이 커지며 느려진 것으로 보이지만 확인 안 했다.
...
**거의 다 n=1 이다.** 커밋된 35회차 중 반복이 있는 건 `ceiling.env` 의 네 rate 뿐이고(두 번씩), 그 반복이 포화 지점의 재현 불가를 드러냈다. 나머지는 한 번씩이다.
