...
| 방식 | 초당 저장 | p95 | 막는 자리 |
| --- | --- | --- | --- |
| **DB 락** | **205/s 에서 포화** | 200 RPS 에서 이미 1.25초 | `events` 행 락 |
| **Redis** | **6,400 RPS 에서 5,761/s, 안 꺾임** | **2.15ms** | 아직 못 찾음 |

**28배 차이고, Redis 쪽은 천장이 아니다.** 6,400 RPS 에서도 요청을 하나도 안 흘리고 2밀리초에 답했다. 한계는 그 위 어딘가다.
...
앱이 유휴인데 p95 가 1초였던 이유다. **같은 앱, 같은 6,400 RPS, 같은 1분. JVM 상태만 다르다.**

| 앱 상태 | p95 | 버려진 요청 | 초당 저장 |
| --- | --- | --- | --- |
| **따뜻함** (38만 건 처리 후) | **2.15ms** | 0 | 5,761 |
| **재기동 직후** | **1,474ms** | 10,562 | 5,607 |

**686배다.** 처리량은 거의 같은데(5,607 대 5,761) 지연만 무너진다.
...
**커밋된 스윕 결과는 전부 콜드 JVM 이다.** `compare`·`drainer`·`ceiling`·`ceiling-narrow` 네 디렉터리가 그렇다. 회차마다 앱을 재기동하기 때문이다. 5 장을 알고 나서 다시 읽어야 한다.
...
> `ceiling`·`ceiling-narrow` 의 지연 수치는 콜드 JVM 이 섞여 있다. 남겨두는 건 5 장의 근거이자, 배포·스케일아웃 중 피크가 어떤 모양인지 보여주기 때문이다. **정상 상태 수치로 인용하면 안 된다.**
