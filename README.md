...
> `warmup` 3회차를 뺀 나머지 32회차 — 스윕 다섯 디렉터리와 단발 회차 하나 — 의 지연 수치는 콜드 JVM 이다. 남겨두는 건 5 장의 근거이자, 배포·스케일아웃 중 피크가 어떤 모양인지 보여주기 때문이다. **정상 상태 수치로 인용하면 안 된다.**
>
> 이후 회차부터는 `result.json` 의 `load.warmed_by` 로 구분된다. `"off"` 는 워밍업을 끄고 일부러 콜드를 잰 것, `"none"` 은 돌렸는데 부하가 안 나간 것, `"unknown"` 은 손으로 `run.sh` 를 돌려 알 수 없는 것, 나머지는 그 길이만큼 데운 것이다.
>
> 길이만으로는 충분한지 알 수 없어 `load.warmup_requests` 도 같이 남는다 — 낮은 rate 에서 30초면 몇 천 건뿐이라 JIT 이 덜 올라간다. 그 숫자가 작으면 웜이라고 적혀 있어도 지연을 믿으면 안 된다. `load.warmup_drain_capped` 가 `true` 면 워밍업이 남긴 걸 다 못 빼고 본 회차가 시작됐다는 뜻이다. 밀린 항목 자체는 상태 초기화가 지우므로 그 회차 `drain.seconds` 에 안 들어온다 — 남는 건 드레이너 경합과 커진 테이블이다.
>
