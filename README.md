...
**커밋된 스윕 결과는 전부 콜드 JVM 이다.** `compare`·`drainer`·`ceiling`·`ceiling-narrow`·`bottleneck` 다섯 디렉터리가 그렇다. 회차마다 앱을 재기동하기 때문이다. 5 장을 알고 나서 다시 읽어야 한다.
...
**거의 다 n=1 이다.** 커밋된 35회차 중 같은 rate 를 두 번 이상 돈 건 `ceiling.env` 의 네 rate 뿐이다 — 800·1600 이 두 번, 3200 이 세 번, 6400 이 네 번. 그 반복이 포화 지점의 재현 불가를 드러냈다. 다만 **조건이 서로 달라 반복 측정이라고 부르기 어렵다** — 부하 길이도 VU 설정도 JVM 상태도 회차마다 다르다.
...
> `warmup` 을 뺀 나머지 — `compare`·`drainer`·`ceiling`·`ceiling-narrow`·`bottleneck` — 의 지연 수치는 콜드 JVM 이다. 남겨두는 건 5 장의 근거이자, 배포·스케일아웃 중 피크가 어떤 모양인지 보여주기 때문이다. **정상 상태 수치로 인용하면 안 된다.**
>
> 이후 회차부터는 `result.json` 의 `load.warmed_by` 로 구분된다. `"none"` 이면 콜드다. 여기 있는 35회차는 그 필드가 생기기 전이라 전부 없다.
